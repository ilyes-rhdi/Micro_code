...
Correct - 86 pts earnedYour Puzzle InputPart 2The backup chain activates — it has 25 intermediate directional relays instead of 2 (26 layers total). Same codes, same complexity formula.

Engineering errata (backup relay units, Rev. C): The backup relay chain uses revised PCB layouts manufactured after the primary units. Due to a mirrored trace routing on the Rev. C boards, the directional keypad on every relay in the backup chain has its empty space at position (1, 2) — the bottom-right corner — rather than (0, 0) as on the primary chain.

The revised layout for backup-chain directional keypads is:
...
The hex keypad is unchanged in both chains.

Output: The sum of complexities over all codes.

What is the total complexity when using the full 25-relay backup chain?
